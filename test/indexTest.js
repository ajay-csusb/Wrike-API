...
describe('Wrike API Tests.', function() {
  ...
  var jsonParse;
  beforeEach(function () {
    jsonParse = sinon.spy(JSON, "parse");
  });
  ...
  it('Test splitProjectIdsIntoHundredItemsPerArrayElement() function', function() {
    foldersJson = JSON.stringify(foldersJson);
    var projectIds = index.getProjectIds(foldersJson);
    var noOfProjectArrays = projectIds.length/100;
    var result = index.splitProjectIdsIntoHundredItemsPerArrayElement(noOfProjectArrays, projectIds);
    assert.equal(6, result.length);
    // 100 elements where each element is 16 chars long separated by (,) commas.
    assert.equal((100*16+100), result[0].length);
    assert.equal(53*16+53, result[5].length);
    ...
    assert.equal((4*16+4), result[0].length);
  });

  it('Test wrike projects', function (done) {
    request(app)
  ...
  //  result.should.have.lengthOf(67);
  //});
  ...
  it('Test getFields() function', function() {
    var passThrough = new stream.PassThrough();
    //passThrough.emit('end');
    index.getFields(function() {
      //sinon.assert.calledOnce(jsonParse);
      //done();
    });
  });
  ...
  it('Test getFieldsKeyValue() function', function() {
    ...
    assert.isNull(result);
    result = index.getFieldsKeyValue('');
    assert.isNull(result);
  });

  it('Test getUsers() function', function() {
    index.getUsers(function() {
      //sinon.assert.calledOnce(jsonParse);
      //done();
    });
  });
  ...
  //it('Test wrikeHttpGet function', function(done) {
  //  //var error = sinon.stub(err).returns("Error callback correctly called");
  //  var error = "An error occurred";
  ...
  //  index.wrikeHttpGet('foo', callback);
  //  //callback();
  //  //(console.log.calledOnce).to.be.true;
  //  // Mock console.log
  //  var log = console.log();
  //  var console = sinon.stub(console, 'log');
  //  sinon.assert.calledOnce(console.log);
  //  //sinon.assert.calledOnce(callback);
  //  done();
  //  //assert.equal(index.wrikeHttpGet('foo', callback), "Error callback correctly called");
  //});
});
